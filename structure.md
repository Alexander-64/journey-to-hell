...
- необходимо было управлять загрузкой библитеки React, асинхронно грузить при переходе на главную, либо при обновлении вкладки страницы реализованной на реакте грузить. В общем, тут явно требовался механизм чтобы управлять всеми такими кейсами
...
- получилось бы нам попасть в заявленные 2 секунды при 1 Mbit/s при развитии кодовой базы? Не уверен. Нужно бы было очень хорошо разобраться в устройстве фреймворка, засплитить все что только можно. С ангуляром было бы проще, но следует понимать, что мы бы все равно грузили большое количество кода (как минимум сразу Angular + Backbone + код самого фреймворка) для отображения только одной услуги при явном переходе к ней по url

В общем, если бы Виктор Франкенштейн увидел этот итоговый результат он бы понял, что его работа это всего лишь возня в детской песочнице. К счастью, в июле месяце после встречи с заказчиком стало ясно, что услуги будут переработаны практически полностью, от редизайна ждут сильно больше чем просто "перекрасить" контролы.
...
В сентябре руководство поставило задачу ускориться по работам над Личным кабинетом. В связи с этим был найм пары человек в штат, пары с других проектов, плюс привлечение 10 подрядчиков - итого 20 frontend разработчиков. В связи с этим команда условно была разделена на 3 стрима.
...
* долгий цикл обратной связи от тестирования - вступали в игру только после релиза 
* не было тестовых сценариев по которым реализовывались тесты командой фронта. Писали как могли
* тесты написанные разработчиками сослужили фронту отличную службу:
* * позволили делать стабильные релизы
* * упростили разработку
* * дали возможность легко проводить рефакторинг
однако оказались не востребованы на уровне команды проекта (могли стать основой для e2e тестов) 

Уроки:
* привлекать тестирование к написанию тест кейсов ДО старта работ по задаче или, как минимум, параллельно. Реализовывать тестовые сценарии в рамках задачи силами разработчиков
* плотнее взаимодействовать с командой тестирования - привлекать тестирование к проверке фич на дев\интеграционном стенде по мере готовности - нивелирует риски:
* * некорректного понимания и реализации требований разработчиками
* * до релиза сообщать о дефекте - разработчик его или исправляет в текущем релизе, или не включает, снижая объем работы в рамках проверки релиза
* тесты фронта выполненные на mock бекенде могут стать хорошей базой для e2e тестов, т.к.:
* * сами тесты уже  написаны (с учетом тест кейсов от самих тестировщиков) - необходимо поправить url и изменить проверки учитывающие реальные ответы от  сервера 
* * e2e тесты на функционал мы получаем вместе с релизом
* тесты фронта могут стать полезным инструментом для тестирования позволяющим локализовать проблему для тестировщиков

Рефлексируя, провели внутренний митап с тестированием